...
public class UserController {
	...
	//add req handling method for users logout
	@GetMapping("/logout")
	public String logOut(HttpSession session, Model model, HttpServletRequest request , HttpServletResponse response)
	{
		...
		model.addAttribute("user_dtls", session.getAttribute("user_dtls"));//adding user details to req scope from session 
		//scope before closing session
		session.invalidate();
		//set refresh header to navigate the client to the home page(index.jsp) after some delay
